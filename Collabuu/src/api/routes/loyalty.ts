...
export class LoyaltyRouter {
  ...
  private async getPoints(req: AuthenticatedRequest, res: Response) {
    try {
  ...
  private async getPointsForBusiness(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
  ...
  private async getTransactions(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
      if (!customerId) {
        return res.status(401).json({ error: 'Unauthorized' });
  ...
  private async getRewards(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
      if (!customerId) {
        return res.status(401).json({ error: 'Unauthorized' });
  ...
  private async redeemReward(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
  ...
  private async getRedemptions(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
      if (!customerId) {
        return res.status(401).json({ error: 'Unauthorized' });
  ...
  private async getRedemptionQR(req: AuthenticatedRequest, res: Response) {
    try {
      const customerId = req.user?.id;
      ...
      // Generate QR code
      const qrCodeImage = await QRCode.toDataURL(redemption.qr_code_data);
