...
export interface VisitStats {
  ...
  rejected: number;
  pending: number;
  byDay: Record<string, number>; // Format: 'YYYY-MM-DD': count
  byInfluencer: Record<string, number>; // Format: influencerId: count
}
...
export class VisitService {
  ...
  async getVisitsByBusiness(businessId: string, filters?: {
    status?: VisitStatus,
    startDate?: Date,
    endDate?: Date
  }): Promise<Visit[]> {
  ...
  async getVisitsByInfluencer(influencerId: string): Promise<Visit[]> {
    const { data, error } = await this.supabase
      .from('visits')
      .select('*')
  ...
  async createVisit(visit: Omit<Visit, 'id' | 'createdAt' | 'approvedAt'>): Promise<Visit | null> {
    const { data, error } = await this.supabase
      .from('visits')
      .insert([{
  ...
  async approveVisit(id: string, creditsEarned: number, loyaltyPointsEarned: number): Promise<Visit | null> {
    const { data, error } = await this.supabase
      .from('visits')
      .update({
        status: 'approved',
  ...
  async rejectVisit(id: string): Promise<Visit | null> {
    const { data, error } = await this.supabase
      .from('visits')
      .update({
        status: 'rejected'
  ...
  async getVisitStats(businessId: string, period?: {
    startDate: Date;
    endDate: Date;
  }): Promise<VisitStats> {
    ...
    for (const visit of visits) {
      // Count by status
      if (visit.status === 'approved') approved++;
      if (visit.status === 'rejected') rejected++;
      if (visit.status === 'pending') pending++;
  ...
  private mapDbVisitToVisit(dbVisit: any): Visit {
    return {
      id: dbVisit.id,
