...
class VisitRouter {
    constructor() {
        ...
        this.router.get('/', auth_1.authenticateToken, this.getAllVisits.bind(this));
        this.router.get('/:id', auth_1.authenticateToken, this.getVisitById.bind(this));
        this.router.post('/:id/validate', auth_1.authenticateToken, this.validateVisit.bind(this));
    }
    async getAllVisits(req, res) {
        ...
        `);
            ...
            if (userType === 'business') {
                ...
                const { data: influencerCampaigns } = await supabase_1.supabase
                    .from('campaigns')
                    .select('id')
    ...
    async validateVisit(req, res) {
        try {
            ...
            // Get the visit
            const { data: visit, error: fetchError } = await supabase_1.supabase
                .from('visits')
                .select(`
    ...
    async processVisitRewards(visit) {
        try {
            const campaign = visit.campaigns;
            ...
            if (visit.referral_code_id) {
                ...
                const { data: referralCode } = await supabase_1.supabase
