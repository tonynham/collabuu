...
class CampaignRouter {
    ...
    async getByDeeplink(req, res) {
        try {
            ...
            const { data: campaign, error } = await supabase_1.supabase
                .from('campaigns')
                .select(`
    ...
    async validateReferralCode(req, res) {
        try {
            ...
            const { data: referral, error } = await supabase_1.supabase
        ...
        `)
            ...
            const influencer = referral.influencer_profiles;
            res.status(200).json({
                valid: true,
                referral: {
                    id: referral.id,
